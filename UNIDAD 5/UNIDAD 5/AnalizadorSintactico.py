...
def main():
    ...
    with open(fileName,"r") as file:
        for line in file: #Linea del archivo
            for word in line.split(): #Palabra en el archivo
                wordList.append(word) #Añadimos lista
    
    if(programa()):
        print("El programa es valido!")
    else:
        print("Programa no es valido :C")

def programa():
    print("------------Programa de analizador sintactico UNIDAD 5---------------")
    ...
    word = wordList.pop(0)
    if(word == "begin"):
        print(word, "{") #Imprime palabra

        #Revisa declaraciones
        declaraciones()
        #print("Palabra lista despues de las declaraciones")
...
def declaraciones():
    #check declaracion | declaracion; siguiente declaracion
    declaracion() 
    if(wordList[0] == ";"): #check de declaracion termina en ;
            print(";")
            wordList.pop(0) #remove ; de la lista de palabras
            next_declaraciones() #check siguiente declaration
    else:
...
def declaracion():
    if(tipo()):
        word = wordList.pop(0)
        print(word, end = ' ') #impresion palabra
        lista_variables()
    else:
